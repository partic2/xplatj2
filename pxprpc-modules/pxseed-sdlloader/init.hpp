...
namespace pxseed_sdlloader{

const char *data_dir=NULL;
std::string get_data_path(const char *subpath){
    #ifdef __ANDROID__
    if(data_dir==NULL){
        data_dir=SDL_GetAndroidInternalStoragePath();
    }
    #else
    if(data_dir==NULL){
        data_dir="./data";
...
void start(){
    ...
    SDL_Log("pxseed_sdlloader:SDLLoader startup");
    {
        //to init wasi module on windows, maybe changed in future.
        FILE *fh=NULL;
        fh=freopen(get_data_path("/stdout.txt").c_str(),"w",stdout);
        if(fh==NULL){
            SDL_Log("pxseed_sdlloader:freopen stdout error %s",strerror(errno));
        }else{
            setvbuf(fh, NULL, _IONBF, 0);
        }
        fh=freopen(get_data_path("/stderr.txt").c_str(),"w",stderr);
        if(fh==NULL){
            SDL_Log("pxseed_sdlloader:freopen stderr error %s",strerror(errno));
        }else{
            setvbuf(fh, NULL, _IONBF, 0);
        }
        ...
        if(f==NULL){
            SDL_Log("pxseed_sdlloader:.wasm file open failed.");
            return;
        }else{
            SDL_Log("pxseed_sdlloader:.wasm file opened.");
        }
        ...
        if(filesize>1024*1024*1024){
            SDL_Log("pxseed_sdlloader:.wasm file too large(>1GB)");
            fclose(f);
            return;
        }
        fseek(f,0,SEEK_SET);
        ...
        SDL_Log("pxseed_sdlloader:pwart namespace created.");
        pwart_syslib_load(ns);
        SDL_Log("pxseed_sdlloader:syslib initialized.");
        len = fread(data, 1, filesize, f);
        SDL_Log("pxseed_sdlloader:.wasm data readed.");
        fclose(f);
        pwart_module_state stat=pwart_namespace_define_wasm_module(ns,const_cast<char *>("__main__"),data,len,&err);
        free(data);
        if(err!=NULL){
            SDL_Log("pxseed_sdlloader:error occur:%s\n",err);
            return;
        }
        SDL_Log("pxseed_sdlloader:.wasm module compiled.");
        struct pwart_wasm_memory *mem=pwart_get_export_memory(stat,const_cast<char *>("memory"));
        pwart_wasi_module_set_wasimemory(mem);
        SDL_Log("pxseed_sdlloader:wasi initialzed.");
        err=pwart_wasi_module_init();
        if(err!=NULL){
            SDL_Log("pxseed_sdlloader:warning:%s uvwasi will not load",err);
        }
        ...
        if(fn!=NULL){
            ...
            SDL_Log("pxseed_sdlloader:%s\n","'_start' function not found. ");
        }
        SDL_Log("pxseed_sdlloader:%s\n",".wasm._start exited. ");
        pwart_free_stack(stackbase);
    }
    
    SDL_Log("pxseed_sdlloader:.wasm exit\n");
    if(logfile!=NULL){
        fclose(logfile);
        logfile=NULL;
    }
...
void init(){
    if(inited)return;
    inited=1;
    pxprpc::defaultFuncMap.add((new pxprpc::NamedFunctionPPImpl1())->init("pxseed_sdlloader.RunBoot0Wasm",
    ...
    ).add((new pxprpc::NamedFunctionPPImpl1())->init("pxseed_sdlloader.SDL_RunMain",
        [](Parameter* para,AsyncReturn* ret)->void{
            //On android , We just send event, handled by Java side, before we implement pxprpc client for C.
            #ifndef __ANDROID__
            pxprpc_rtbridge_host::runInNewThread([ret]()-> void {
                SDL_RunApp(0,nullptr,&SDL_main,nullptr);
            });
            #endif
            uv_timeval64_t tv;
            uv_gettimeofday(&tv);
            pxprpc_rtbridge_base::rtbvarsSet("pxseed_sdlloader.event.SDL_RunMain", std::to_string(tv.tv_sec*1000+tv.tv_usec/1000));
            ret->resolve();
        })
    ).add((new pxprpc::NamedFunctionPPImpl1())->init("pxseed_sdlloader.SetWasmSdlEventHandler",
        [](Parameter* para,AsyncReturn* ret)->void{
            auto wasmfn=para->nextLong();
            sdlEventHandle=reinterpret_cast<pwart_wasm_function>(wasmfn);
            ret->resolve();
        })
    );
    //sdlEventHandle
...
extern "C"{
    ...
    extern int SDL_main(int argc, char *argv[]){
        SDL_Init(SDL_INIT_AUDIO|SDL_INIT_VIDEO|SDL_INIT_EVENTS|SDL_INIT_CAMERA|SDL_INIT_SENSOR);
        pxseed_sdlloader::sdlloop();
        SDL_Quit();
        return 0;
    }
}